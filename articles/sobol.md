...
The **sobol** package generates low‑discrepancy Sobol sequences and is
designed from the ground up for **parameter space exploration**.  
This tutorial will take you from the simplest possible usage to advanced
...
Imagine you are tuning a machine‑learning model with three
hyperparameters:
...
You want to explore the space with 200 well‑spread points.
...
The design is **deterministic** and **space‑filling** – already a big
...
to generate a **Sobol sequence** in the unit cube and then scales each
column to your bounds.

- **Low discrepancy** – points are more evenly distributed than
  random.  
- **Reproducible** – you’ll get the exact same design every time.  
- **No wasted points** – every evaluation adds information about the
...
If you already have your own scaling logic, or need the raw `[0,1)`
points, use
[`sobol_points()`](https://alrobles.github.io/sobol/reference/sobol_points.md)
directly.
...
[`sobol_points()`](https://alrobles.github.io/sobol/reference/sobol_points.md)
accepts an optional `skip` argument that lets you start from an
arbitrary index – perfect for parallel workers (see below).

## 4. Incremental generation with `sobol_generator()`

Sometimes you don’t know in advance how many points you’ll need. Maybe
you want to **evaluate a few, check convergence, then generate more**.
...
All sequences are deterministic. So two calls with the same parameters
will always match:
...
To distribute work across multiple cores or machines, assign each a
**non‑overlapping skip interval**.
...
*(A `skip` argument for
[`sobol_design()`](https://alrobles.github.io/sobol/reference/sobol_design.md)
is under consideration – once available, parallel designs become
one‑liners.)*
...
The C++ engine is heavily optimised. Even 1 000 000 points in 10
dimensions complete in under a second on modern hardware, freeing you to
spend time on your actual model.

For extremely high dimensions (\>1000) the engine falls back to runtime
generation – still fast, but initialisation takes a tick longer.
Precomputed tables cover the first 1000 dimensions instantly.
...
- **Articles** – “Performance Optimisation” and “Testing Strategy” on
  the package website  
- **Real‑world example** – check the `inst/examples/usage_examples.R`
  file
...
That’s all you need to start exploring your parameter space smarter and
faster.  
**Welcome to `sobol`!**
